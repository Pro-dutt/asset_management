...
import deviceCategoryTableConstants from "./constants";
import TableIcon from "@/components/table/utils/icon";

class DeviceCategoryTableUtils {
    static tableHeader({ data, setShow, styles, hasPermission }) {
        ...
        const url = deviceCategoryTableConstants.TABLE_API_URL;
        return {
            limit: deviceCategoryTableConstants.TABLE_LIMITS,
            actionButtons: [
                hasPermission(url, "POST") && {
                    variant: "primary",
                    icon: TableIcon.PLUS,
                    label: "Add New Device Category",
    ...
    static tableActionData({ data, setShow, setDeviceCategoryDetails, hasPermission }) {
        const handleAction = (row, key) => {
            setDeviceCategoryDetails(data?.data?.find((item) => row["Id"].value === item._id));
            setShow({ [key]: true });
        };
        const url = deviceCategoryTableConstants.TABLE_API_URL;
        return [
...
export default DeviceCategoryTableUtils;
