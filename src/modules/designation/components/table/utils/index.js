...
import designationTableConstants from "./constants";
import TableIcon from "@/components/table/utils/icon";

class DesignationTableUtils {
    static tableHeader({ data, setShow, styles, hasPermission }) {
        ...
        const url = designationTableConstants.TABLE_API_URL;
        return {
            limit: designationTableConstants.TABLE_LIMITS,
            actionButtons: [
    ...
    static tableActionData({ data, setShow, setDesignationDetails, hasPermission }) {
        const handleAction = (row, key) => {
            setDesignationDetails(data?.data?.find((item) => row["Id"].value === item._id));
            setShow({ [key]: true });
        };
        const url = designationTableConstants.TABLE_API_URL;
        return [
...
export default DesignationTableUtils;
